...
function Currency() {
    ...
    useEffect(() => {
        ...
        axios.get(
            `https://api.timezonedb.com/v2.1/list-time-zone?key=EURQ74A0QNOD&format=json&zone=${currencyCodes.join(',')}`
        )
            .then(response => {
                const timeZones = {};
                response.data.zones.forEach(zone => {
                    timeZones[zone.zoneName] = new Date().toLocaleTimeString('en-US', {
                        timeZone: zone.zoneName,
                        hour12: false,
                    });
                });
                setRates(prevRates => ({ ...prevRates, timeZones }));
            })
            .catch(error => {
                console.log(error);
            });
